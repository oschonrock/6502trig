...
; i had this code to test it, but it didn't really do what I wanted
...
;             #scr.setpos 2, 9
; 	    pla
;             jsr scr.hex8out  ; report error condition
...
Keyboard .block 
    ...
    ; ZERO PAGE Varibles
    ScanResult       = $50  ; 8 bytes
    BufferNew        = $58  ; 3 bytes
    KeyQuantity      = $5b  ; 1 byte
    NonAlphaFlagX    = $5c  ; 1 byte
    NonAlphaFlagY    = $5d  ; 1 byte
    TempZP           = $5e  ; 1 byte
    SimultaneousKeys = $5f  ; 1 byte
...
Main
   ...
   +
    ...
    .for i := 6, i != -1, i -= 1
        ldx #[7-i]*8
        lda ScanResult+i
        cmp #$ff                ; BUG, missing in 2.5, fixed in 2.6
        beq *+5
            jsr KeyInRow
    .endfor
